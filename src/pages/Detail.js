...
const Detail = ({ currentMeal, loading }) => {
  ...
  const ingredients = meal && meal.ingredients && meal.ingredients.map((value, idx) => {
    return (
      <p key={idx} className={"detail-info-item"}>{`${value} ${meal.measures[idx]}`}</p>
    )
  });

  const tags = meal && meal.tags && meal.tags.split(',').map((tag, idx) => {
    return (
      <Badge key={idx} color="info" className={"detail-tag"} style={{ paddingTop: '10px' }}>{tag}</Badge>
    )
  });
